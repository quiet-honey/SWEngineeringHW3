...
ApplyInfo Member::removeApplyInfo(string bizNum) {
    ...
    for (auto it = applyInfos.begin(); it != applyInfos.end(); it++) {
        if (it->getBizNum() == bizNum) {
            cancelledInfo.setCompanyName(it->getCompanyName());  // 회사 이름 저장
            cancelledInfo.setbizNum(it->getBizNum());  // bizNum 저장
            cancelledInfo.setWork(it->getWork());  // 작업 저장
            cancelledInfo.setNumOfPeople(it->getNumOfPeople());  // 인원 수 저장
            cancelledInfo.setDeadline(it->getDeadline());  // 마감일 저장
            applyInfos.erase(it);
            break;
        }
    }
