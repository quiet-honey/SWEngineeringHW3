...
void selectApplyInfo() {
    //해당 기능 수행
    vector<ApplyInfo> applyInfos = ApplyInquiryUI::selectApplyInfo();

    // 출력 형식
    fprintf(out_fp, "4.3 지원 정보 조회\n");
    for (ApplyInfo info : applyInfos) {
        fprintf(out_fp, "%s %s %s %d %s\n", info.getCompanyName().c_str(), info.getBizNum().c_str(), info.getWork().c_str(), info.getNumOfPeople(), info.getDeadline().c_str());
    }
};
...
void cancelApplyInfo() {
    ...
    // 입력 형식 : 사업자번호를 파일로부터 읽음
    fscanf(in_fp, "%s", bizNum);

    //해당 기능 수행
    ApplyInfo applyInfos = CancelApplyInfoUI::cancelApplyInfo(bizNum);

    // 출력 형식
    fprintf(out_fp, "4.4 지원 취소\n");
    fprintf(out_fp, "%s %d %s\n", applyInfos.getCompanyName().c_str(), applyInfos.getBizNum().c_str(), applyInfos.getWork().c_str());
};
...
void showRecruitNumInfo() {
    ...
    map<string, int> applyNum = GetApplyNumUI::showApplyNumInfo();

    // 출력 형식
    fprintf(out_fp, "5.1 지원 정보 통계\n");
    for (const auto& pair : applyNum) {
